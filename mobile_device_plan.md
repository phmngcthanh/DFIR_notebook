...
| **P4 — Device enablement** (closes T4–T7) | `tauri android init` (SDK host), push branch → run `android-build.yml` → APK artifact, `adb install` on the Tab S9 FE, on-device fix round, keystore for release builds | 🟡 half done (2026-07-20): branch pushed, CI proven — run 29754986587 built a 43.6 MB debug APK for `aarch64-linux-android`, including the SQLCipher + vendored-OpenSSL cross-compile (T4, T5 closed; needed an NDK r23+ binutils shim, see workflow). Remaining: sideload on the Tab S9 FE, on-device fix round, release keystore (T6, T7) |
